    ...
    for i in range(frameCount):
        if (i % sampling_rate) == 0:
            ...
            for j in range(3):
                ...
                sampled_frame[i // sampling_rate, :, :, j] = (sampled_frame[i // sampling_rate, :, :, j])
...
def detect_net():
    net = model_zoo.get_model('yolo3_mobilenet1.0_voc', pretrained=True,ctx=gpu(0))
...
def detect(net, frame_image):
    ...
    i=0
    final_bounding_box = bounding_boxs[0][0]
    area_max = 0
    for IDs in class_IDs[0]:
        if IDs == 14 and scores[0][i] > 0.5:
            [xmin, ymin, xmax, ymax]=bounding_boxs[0][i]
            area = (xmax-xmin)*(ymax-ymin)*scores[0][i]
            if area > area_max:
                area_max = area
...
vid_file_name = "VID_20200315_175032"
...
for current_frame in frames:
    ...
    # write bounding box
        ...
        data_json['sleep'].append([float(i * 1 / 40), float(scores)])
    else:
        data_json['sleep'].append([float(i * 1 / 40), float(0)])
...
#skvideo.io.vwrite(vid_file_name + "detection" + ".mp4", frames_vid,inputdict={},outputdict={'-r': str(30)})
